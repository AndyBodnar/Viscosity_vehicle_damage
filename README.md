...
- **Fuel leak → starvation → death.** A ruptured tank trails fuel, drains, and eventually starves the engine. It can catch fire if it's critical and still running.
- **Staged engine failure.** Healthy → running rough (power loss) → sputtering/stalling → dead. You get a window to limp somewhere before it gives out.
